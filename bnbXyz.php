...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0)
{
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, "bnbxyz.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, "bnbxyz.txt");
...
if ($IP != "") {
    ...
    $login  = login($email, $token, array_merge(head(), array("Referer: https://bnbpayu.xyz/")));
    sleep(5);
    ...
    while (true) {
        for ($i = 0; $i < 500; $i++) {
            ...
            if ($check == "404") {
                ...
                if ($sttsBal == "Empty") {
                    echo $check2;
                    echo $WhiteTebal . "[5] Coin " . $YellowTebal . "BNB PAYU XYZ " . $RedTebal . "Kosong. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                    sleep(2);
                    system('php dashtop.php');
                } else {
